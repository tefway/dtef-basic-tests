...
inline std::string fillField(const std::string& value, size_t len) {
    if (value.size() >= len) return value.substr(0, len);
    return value + std::string(len - value.size(), '0');
}

// Helper to build a test record string for a given type
inline std::string buildTestRecord(const std::string& tipoRegistro, const std::vector<FieldDef>& fields) {
    std::string s;
    s += fillField("1234", 4); // NumeroEmpresa
    s += fillField("567890", 6); // NumeroLoja
    s += fillField(tipoRegistro, 3); // TipoRegistro
    for (const auto& f : fields) {
        s += fillField(f.name.substr(0, std::min<size_t>(f.name.size(), f.length)), f.length);
    }
    return s;
}
...
TEST(ParseLogTest, parseTransacaoParaJson_CCR) {
    ...
    EXPECT_EQ(obj->getValue<std::string>("NumeroCartao"), fillField("NumeroCartao", 37));
...
TEST(ParseLogTest, parseTransacaoParaJson_CDB) {
    ...
    EXPECT_EQ(obj->getValue<std::string>("NumeroCartao"), fillField("NumeroCartao", 37));
    EXPECT_EQ(obj->getValue<std::string>("ValorTaxaEmbarque"), fillField("ValorTaxaEmbarque", 12));
}
...
TEST(ParseLogTest, parseTransacaoParaJson_CCD) {
    ...
    EXPECT_EQ(obj->getValue<std::string>("Reservado1"), fillField("Reservado1", 1));
    EXPECT_EQ(obj->getValue<std::string>("Reservado3"), fillField("Reservado3", 55));
}
...
TEST(ParseLogTest, parseTransacaoParaJson_PrivateLabel) {
    ...
    EXPECT_EQ(obj->getValue<std::string>("PlanoPagamento"), fillField("PlanoPagamento", 5));
    EXPECT_EQ(obj->getValue<std::string>("CodigoItem"), fillField("CodigoItem", 11));
}
...
TEST(ParseLogTest, parseTransacaoParaJson_QRC) {
    ...
    EXPECT_EQ(obj->getValue<std::string>("NumeroControleRede"), fillField("NumeroControleRede", 80));
    EXPECT_EQ(obj->getValue<std::string>("MetodoPagamento"), fillField("MetodoPagamento", 25));
}

TEST(ParseLogTest, parseTransacaoParaJson_UnknownType) {
    std::string record = fillField("1234", 4) + fillField("567890", 6) + fillField("ZZZ", 3) + "EXTRAFIELDS";
    Poco::JSON::Object::Ptr obj = new Poco::JSON::Object;
